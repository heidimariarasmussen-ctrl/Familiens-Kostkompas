...
function recipeContextDetails(r){if(!recipeViewContext||recipeViewContext.recipeId!==r.id)return {label:r.portion,ingredients:r.ingredients,note:''};const ids=recipeViewContext.consumerIds||[],adults=ids.filter(x=>x==='alex'||x==='heidi').length,kids=ids.includes('kids');const data=portionData[r.id];const names=ids.map(id=>flexPeople.find(p=>p.id===id)?.name).filter(Boolean);let ingredients=r.ingredients,label=names.join(' + ')||r.portion,note='';if(data){if(adults===2&&kids)ingredients=data.family||r.ingredients;else if(adults===1&&!kids)ingredients=data.adult||r.ingredients;else if(adults===0&&kids)ingredients=data.kids2||r.ingredients;else if(adults===2&&!kids)ingredients=combinePortionLines([data.adult,data.adult]);else if(adults===1&&kids)ingredients=combinePortionLines([data.adult,data.kids2]);note='Mængderne er automatisk tilpasset de personer, der spiser denne ret i madplanen.'}else if(r.isCustom){note='Denne egen ret har ikke kuraterede portionsmængder endnu, så ingredienslisten vises som den er skrevet.'}return {label,ingredients,note}}
...
function renderRecipe(id){const r=recipes.find(x=>x.id===id);if(!r){library();return}const is=favs().includes(id),ctx=recipeContextDetails(r),ctxNote=ctx.note?`<div class="note portion-context-note"><b>Planlagt portion: ${escapeHtml(ctx.label)}</b><br>${ctx.note}</div>`:'';app.innerHTML=`<div class="shell">${siteHeader()}${recipeBackButton(r)}<article class="recipe"><button class="heart" onclick="toggleFav('${id}')">${is?'♥':'♡'}</button>${r.isCustom?`<div class="recipe-custom-actions"><button class="btn secondary small" onclick="customRecipeEditor('${id}')">Redigér egen ret</button><button class="text-link danger" onclick="deleteCustomRecipe('${id}')">Slet</button></div>`:''}${recipeMedia(r,'recipe-hero')}<span class="badge">${r.category}</span>${r.isCustom?'<span class="custom-badge">Familiens egen</span>':''}<h1>${escapeHtml(r.name)}</h1><div class="info"><div><b>Portion</b><br>${escapeHtml(ctx.label)}</div><div><b>Aktiv tid</b><br>${r.active}</div><div><b>Samlet tid</b><br>${r.total}</div></div>${ctxNote}<div class="portionbar"><button class="${scale===1?'active':''}" onclick="setScale('${id}',1)">${recipeViewContext?'Planlagt portion':'Kun i dag'}</button><button class="${scale===2?'active':''}" onclick="setScale('${id}',2)">Dobbelt portion</button></div><h3>Ingredienser ${scale===2?'· dobbelt portion':''}</h3><ul>${ctx.ingredients.map(x=>`<li>${scaleIngredient(x,scale)}</li>`).join('')}</ul><h3>Sådan gør du</h3><ol>${r.steps.map(x=>`<li>${x}</li>`).join('')}</ol>${r.taste?`<div class="note"><b>Sovs / dip / smag</b><br>${r.taste}</div>`:''}${r.child?`<div class="note pink" style="margin-top:10px"><b>Til børn på ca. 15 måneder</b><br>${r.child}</div>`:''}${r.tip?`<div class="note" style="margin-top:10px"><b>Praktisk tip</b><br>${r.tip}</div>`:''}<h3>Hvorfor er den god for børnene?</h3><ul>${(r.why||[]).map(x=>`<li>${x}</li>`).join('')}</ul>${feedbackHtml(id)}</article></div>${nav()}`;initFeedbackSelection(id)}
