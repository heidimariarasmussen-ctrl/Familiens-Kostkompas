...
function applyFlexCopy(dayIndex,key,source='current'){const plan=flexPlanForSource(source);if(!plan)return;const targets=[...document.querySelectorAll('.flex-copy-target:checked')].map(x=>Number(x.value));const src=plan.items[dayIndex];targets.forEach(i=>{const dst=plan.items[i];if(!dst)return;if(key==='__day__'){PLAN_MEAL_KEYS.forEach(k=>{dst[k]=src[k];dst.flex=dst.flex||{};dst.flex[k]=cloneFlexMealState(flexMealState(src,k))});dst.leftoverLunch=src.leftoverLunch;dst.makeDouble=src.makeDouble}else{dst[key]=src[key];dst.flex=dst.flex||{};dst.flex[key]=cloneFlexMealState(flexMealState(src,key))}});if(source!=='draft')persistPlan();closeManualPicker();rerenderFlexSource(source)}
...
function flexPersonMealCell(st,p,dayIndex,key,source){const id=st.assignments[p.id];if(id===FLEX_OUT)return `<div class="flex-person-meal is-out"><div class="flex-out-name">🍽️ Spiser ude</div><button class="mini-btn meal-out-btn active" onclick="toggleFlexPersonOut(${dayIndex},'${key}','${p.id}','${source}')">↩ Spis hjemme</button></div>`;const r=flexRecipe(id);return `<div class="flex-person-meal"><button class="flex-recipe-link" onclick="${r?`showFlexRecipe(${dayIndex},'${key}','${p.id}','${source}')`:`openFlexPicker(${dayIndex},'${key}','${p.id}','${source}')`}">${escapeHtml(r?.name||'Vælg ret')}</button><div class="flex-person-tools"><button class="mini-btn" onclick="openFlexPicker(${dayIndex},'${key}','${p.id}','${source}')">Vælg</button><button class="mini-btn meal-out-btn" onclick="toggleFlexPersonOut(${dayIndex},'${key}','${p.id}','${source}')">🍽 Spiser ude</button></div></div>`}
function flexMealRow(d,dayIndex,key,source='current'){const st=flexMealState(d,key);if(st.mode==='shared'){const r=flexRecipe(st.sharedId)||d[key];return `<div class="flex-meal-row shared"><div class="flex-meal-label">${keyLabel(key)}</div><div class="flex-shared-cell"><button class="flex-recipe-link" onclick="showFlexRecipe(${dayIndex},'${key}','shared','${source}')">${escapeHtml(r?.name||'Ikke planlagt')}</button><div class="flex-row-tools"><button class="mini-btn" onclick="openFlexPicker(${dayIndex},'${key}','shared','${source}')">Vælg ret</button><button class="mini-btn choose-btn" onclick="splitFlexMeal(${dayIndex},'${key}','${source}')">Tilpas / spiser ude</button>${flexCopyButton(dayIndex,key,source)}</div></div></div>`}
...
function toggleFlexPersonOut(dayIndex,key,personId,source='current'){const plan=flexPlanForSource(source);if(!plan)return;const d=plan.items[dayIndex],st=flexMealState(d,key);if(st.mode==='shared'){const id=st.sharedId||d[key]?.id||'';st.mode='individual';st.assignments={alex:id,heidi:id,kids:id}}if(!st.lastHome)st.lastHome={};if(st.assignments[personId]===FLEX_OUT){st.assignments[personId]=st.lastHome[personId]||st.sharedId||d[key]?.id||''}else{st.lastHome[personId]=st.assignments[personId]||st.sharedId||d[key]?.id||'';st.assignments[personId]=FLEX_OUT}if(source!=='draft')persistPlan();rerenderFlexSource(source)}
function splitFlexMeal(dayIndex,key,source='current'){const plan=flexPlanForSource(source);if(!plan)return;const d=plan.items[dayIndex],st=flexMealState(d,key),id=st.sharedId||d[key]?.id||'';st.mode='individual';st.assignments={alex:id,heidi:id,kids:id};if(source!=='draft')persistPlan();rerenderFlexSource(source)}
function collapseFlexMeal(dayIndex,key,source='current'){const plan=flexPlanForSource(source);if(!plan)return;const d=plan.items[dayIndex],st=flexMealState(d,key);const ids=Object.values(st.assignments).filter(id=>id&&id!==FLEX_OUT);const shared=ids[0]||st.sharedId||d[key]?.id||'';st.mode='shared';st.sharedId=shared;st.assignments={alex:shared,heidi:shared,kids:shared};d[key]=flexRecipe(shared)||d[key];if(source!=='draft')persistPlan();rerenderFlexSource(source)}
