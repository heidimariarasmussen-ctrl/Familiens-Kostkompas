...
function collapseFlexMeal(dayIndex,key,source='current'){const plan=flexPlanForSource(source);if(!plan)return;const d=plan.items[dayIndex],st=flexMealState(d,key);const ids=Object.values(st.assignments).filter(Boolean);const shared=ids[0]||d[key]?.id||'';st.mode='shared';st.sharedId=shared;st.assignments={alex:shared,heidi:shared,kids:shared};d[key]=flexRecipe(shared)||d[key];if(source!=='draft')persistPlan();rerenderFlexSource(source)}
