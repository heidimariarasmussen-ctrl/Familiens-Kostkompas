...
function generatePlan(){const days=Number(document.getElementById('days').value),busy=document.getElementById('busy').value;localStorage.setItem('kostkompas-plan-settings',JSON.stringify({days,busy}));const pattern=patterns[days];const used=new Set();const out=[];for(let i=0;i<days;i++){const dinner=chooseDinner(pattern[i],used,busy,i);used.add(dinner.id);let lunch,leftover=false;if(i===0){const ids=firstLunchByBusy[busy];lunch=rBy('Frokost',ids[i%ids.length])}else{const prev=out[i-1].dinner;const ln=lunchMap[prev.number]||1;lunch=rBy('Frokost',ln);leftover=true;out[i-1].makeDouble=true}const bIds=breakfastByBusy[busy];const breakfast=rBy('Morgenmad',bIds[i%bIds.length]);const snack=pickSnack([breakfast,lunch,dinner],i);out.push({day:i+1,breakfast,lunch,dinner,snack,leftoverLunch:leftover,makeDouble:false})}currentPlan={days,busy,items:out,created:Date.now()};localStorage.setItem('kostkompas-current-plan',JSON.stringify(currentPlan));renderCurrentPlan()}
...
function mealRow(label,r,note='',dayIndex=null,key=null){
  return `<div class="meal-row">
    ...
    <div class="meal-tools">
      ...
      ${dayIndex!==null&&key?`<button class="mini-btn" onclick="swapMeal(${dayIndex},'${key}')">↻ Byt ret</button>`:''}
    </div>
  </div>`;
}
...
function swapMeal(dayIndex,key){
  ...
  const d=currentPlan.items[dayIndex];
  const current=d[key];
  const next=randomOther(candidatesForMeal(dayIndex,key),current?.id);
  if(!next)return;
  ...
  if(key==='dinner'){
    if(dayIndex<currentPlan.items.length-1){
      d.makeDouble=true;
      const nextDay=currentPlan.items[dayIndex+1];
      const ln=lunchMap[next.number]||1;
      nextDay.lunch=rBy('Frokost',ln)||rBy('Frokost',1);
      nextDay.leftoverLunch=true;
    }else{
      d.makeDouble=false;
    }
  }
  ...
  if(key==='breakfast'||key==='lunch'||key==='dinner'){
    d.snack=pickSnack([d.breakfast,d.lunch,d.dinner],dayIndex+Math.floor(Math.random()*5));
  }

  localStorage.setItem('kostkompas-current-plan',JSON.stringify(currentPlan));
  renderCurrentPlan();
}

function renderCurrentPlan(){
  if(!currentPlan){planner();return}
  const f=nutritionCheck(currentPlan);
  app.innerHTML=`<div class="shell">
    ...
    <div class="engine-note"><b>Byt kun det, der ikke passer.</b> Brug “Byt ret” på et enkelt måltid. Resten af planen bevares. Hvis du bytter en aftensmad, opdateres næste dags restefrokost automatisk.</div>
    <div class="week">
      ${currentPlan.items.map((d,i)=>`<section class="day-card">
        <div class="day-head"><h3>${dayName(d.day)}</h3><span class="day-tag">${currentPlan.busy==='high'?'Høj travlhed':currentPlan.busy==='low'?'God tid':'Normal hverdag'}</span></div>
        ...
        ${mealRow('Aftensmad',d.dinner,d.makeDouble?'Lav dobbelt: aftensmad i dag → frokost i morgen':'',i,'dinner')}
        ${mealRow('Mellemmåltid',d.snack,'',i,'snack')}
      </section>`).join('')}
    </div>
    <div class="section-title"><h2>Ernæringsfilter</h2></div>
...
function nutritionCheck(plan){const ds=plan.items.map(x=>x.dinner);const all=plan.items.flatMap(x=>[x.breakfast,x.lunch,x.dinner,x.snack]).filter(Boolean);const text=all.map(r=>(r.name+' '+r.ingredients.join(' ')+' '+r.why.join(' ')).toLowerCase()).join(' ');const dinnerText=ds.map(r=>r.name.toLowerCase()).join(' ');const fish=ds.filter(r=>/laks|fisk|torsk|sej/.test(r.name.toLowerCase())).length;const fatFish=ds.filter(r=>/laks/.test(r.name.toLowerCase())).length;const ironDays=plan.items.filter(d=>[d.breakfast,d.lunch,d.dinner,d.snack].some(r=>/oksekød|kød|æg|laks|fisk|havre|linser|bønner|rugbrød/i.test((r.name+' '+r.ingredients.join(' '))))).length;return [
{label:'Jern hver dag',ok:ironDays===plan.days,text:`Jernkilder optræder på ${ironDays} af ${plan.days} dage.`},
{label:'C-vitamin ved plantejern',ok:/bær|kiwi|tomat|passata|peberfrugt|broccoli/.test(text),text:'Planen kombinerer relevante plantejernsmåltider med frugt, bær eller grønt med C-vitamin.'},
{label:'Fisk og omega-3',ok:fatFish>=1 && (plan.days<5?fish>=1:fish>=2),text:`${fish} fiskeaftener, heraf ${fatFish} med laks/fed fisk.`},
{label:'Energi nok',ok:/kartoffel|pasta|ris|havre|rugbrød|banan|tortilla|brød/.test(text),text:'Måltiderne indeholder gennemgående tydelige energikilder.'},
{label:'Fedtvariation',ok:/evoo|olivenolie/.test(text)&&/avocado|peanutbutter|tahin|laks|smør/.test(text),text:'Olivenolie kombineres med bl.a. avocado, nøddesmør, fisk, æg, smør eller mejeri.'},
{label:'Mejeri i balance',ok:true,text:'Mejeri indgår som del af måltiderne og er ikke planens eneste protein-/fedtkilde.'},
{label:'Variation over ugen',ok:new Set(ds.map(r=>dinnerRole(r))).size>=Math.min(3,plan.days),text:'Aftensmaden varierer mellem flere proteintyper og måltidsroller.'},
{label:'Salt og forarbejdning',ok:true,text:'Planen er bygget af Kostkompas-opskrifter; børneportioner tilpasses fortsat for salt.'},
{label:'Alderssikkerhed',ok:true,text:'Brug opskrifternes konkrete 15-måneders-tilpasninger ved servering.'},
...
function shoppingList(){
  ...
  app.innerHTML=`<div class="shell">
    ...
    <div class="plan-actions"><button class="btn secondary" onclick="clearShoppingChecks()">Nulstil flueben</button></div>
    <p class="muted">Mængder er samlet på tværs af planen. Synonymer som A38-varianter, EVOO/olivenolie og nogle produktnavne samles nu bedre.</p>
    <div class="shopping-wrap">
      <div class="shop-card">
        ${order.filter(c=>groups[c]?.length).map(c=>`<div class="shop-cat"><h4>${c}</h4>
          ${groups[c].sort((a,b)=>a.name.localeCompare(b.name)).map((v,i)=>{
            const key=`shop:${c}:${v.name}:${v.unit}`;
            return `<label class="shop-item"><input type="checkbox" ${checked[key]?'checked':''} onchange="saveShoppingCheck('${encodeURIComponent(key)}',this.checked)"><span>${fmtItem(v)}</span></label>`;
          }).join('')}
        </div>`).join('')}
      </div>
      <aside class="shop-card"><h3>Tjek basislager</h3>
        <div class="basis-list">${basis.map((x,i)=>{
          const key=`basis:${i}:${x}`;
          return `<label><input type="checkbox" ${checked[key]?'checked':''} onchange="saveShoppingCheck('${encodeURIComponent(key)}',this.checked)"><span>${x}</span></label>`;
        }).join('')}</div>
        <div class="engine-note" style="margin-top:18px"><b>Praktisk kontrol:</b><br>Se køleskab, fryser og skab igennem før indkøb. Flueben gemmes på denne enhed.</div>
      </aside>
    </div>
  </div>${nav()}`;
}
...
fetch('recipes.json').then(r=>r.json()).then(d=>{recipes=d;restorePlan();home()});
