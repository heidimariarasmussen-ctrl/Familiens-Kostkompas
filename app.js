...
async function refreshCloudSession(){
  ...
  try{
    ...
    if(!r.ok){if(r.status===400||r.status===401)saveCloudSession(null);return false} const d=await r.json(); d.expires_at=Math.floor(Date.now()/1000)+(d.expires_in||3600); saveCloudSession(d); return true;
  }catch(e){console.warn('Kunne ikke forny login lige nu',e);return !!cloudSession?.access_token}
}
...
const siteHeader=()=>`<header class="site-header"><button class="site-brand" onclick="home()" aria-label="Gå til forsiden"><span class="logo-mark" aria-hidden="true"><svg viewBox="0 0 64 64" role="img"><circle cx="32" cy="32" r="29" fill="#fffaf2" stroke="#245045" stroke-width="2.5"/><path d="M32 49c-7-5-14-10-14-18 0-5 3-8 7-8 3 0 6 2 7 5 2-3 4-5 8-5 4 0 7 3 7 8 0 8-8 13-15 18z" fill="#de7d58"/><path d="M31 25c-7-1-12-6-13-13 7 1 12 6 13 13z" fill="#6e8f74"/><path d="M34 24c1-7 6-12 13-13-1 7-6 12-13 13z" fill="#3f6b58"/><path d="M32 26V14" stroke="#245045" stroke-width="2" stroke-linecap="round"/></svg></span><span class="site-brand-copy"><strong>Familiens Kostkompas</strong><small>Næringstæt · realistisk · børnevenlig familiemad</small></span></button>${cloudBadge()}</header>`;
...
function card(r){return `<article class="card" onclick="showRecipe('${r.id}')">${recipeMedia(r)}<div class="card-body"><span class="badge">${r.category}</span>${r.personalOwner?`<span class="custom-badge">${r.personalOwner==='alex'?'Alex-favorit':'Heidi-favorit'}</span>`:''}${r.comfortOption?'<span class="custom-badge">Kun Alex + Heidi</span>':''}${r.isCustom?'<span class="custom-badge">Egen ret</span>':''}<h3>${escapeHtml(r.name)}</h3><div class="meta">${r.active||''} aktiv · ${r.total||''}</div></div></article>`}
function home(){const hasPlan=!!currentPlan?.items?.length;app.innerHTML=`<div class="shell">${siteHeader()}<section class="hero"><div class="hero-copy"><h1>Hvad skal vi spise?</h1><p>80 familieopskrifter + personlige voksenfavoritter + jeres egne retter + Børnefavoritter 2.0. Find en ret nu, gem favoritter eller lad Kostkompasset lave en personlig madplan med rester og indkøbsliste.</p><div class="actions home-actions"><button class="btn home-action action-plan" onclick="${hasPlan?'renderCurrentPlan()':'planner()'}"><span>📅</span>Nuværende madplan</button><button class="btn home-action action-quick" onclick="quickNow()"><span>⚡</span>Jeg skal bruge noget hurtigt nu</button><button class="btn home-action action-shopping" onclick="${hasPlan?'shoppingList()':'planner()'}"><span>🛒</span>Indkøbsliste</button><button class="btn home-action action-today" onclick="planToday()"><span>✓</span>Planlæg i dag</button><button class="btn home-action action-recipes" onclick="library()"><span>📖</span>Se alle opskrifter</button><button class="btn home-action action-pantry" onclick="pantry()"><span>🏡</span>Basislager</button></div>${hasPlan?'':`<p class="home-plan-hint">Du har ingen nuværende madplan endnu. Tryk på <b>Nuværende madplan</b> for at lave den første.</p>`}</div><div class="hero-art"><img src="familien-forside.png" alt="Familien samlet omkring spisebordet"></div></section><div class="home-section-head"><span class="eyebrow">Gå på opdagelse</span><h2>Find efter måltid</h2></div><div class="grid meal-category-grid">${["Morgenmad","Frokost","Aftensmad","Mellemmåltider"].map(c=>`<div class="cat cat-${c.toLowerCase().replace('å','a').replace('ø','o')}" onclick="library('${c}')"><span>${emoji[c]}</span><h3>${c}</h3><div class="small">${recipes.filter(r=>r.category===c).length} opskrifter</div></div>`).join('')}</div><div class="section-title inspiration-title"><div><span class="eyebrow">Fra opskriftsbiblioteket</span><h2>Inspiration</h2></div><button class="btn secondary" onclick="library('Børnefavoritter 2.0')">Børnefavoritter 2.0</button></div><div class="cards">${pickInspiration().map(card).join('')}</div></div>${nav()}`}
function pickInspiration(){return ['aftensmad-1','morgenmad-4','frokost-15'].map(id=>recipes.find(r=>r.id===id)).filter(Boolean)}
function library(cat){currentCategory=cat||null;const rs=cat?recipes.filter(r=>r.category===cat):recipes;app.innerHTML=`<div class="shell">${siteHeader()}<div class="section-title"><h2>${cat||'Alle opskrifter'}</h2><div class="actions"><button class="btn secondary" onclick="customRecipeEditor()">➕ Egen ret</button><button class="btn secondary" onclick="home()">← Tilbage</button></div></div><input class="search" id="q" placeholder="Søg efter ret eller ingrediens…" oninput="filterList()"><div class="cards" id="cards">${rs.map(card).join('')}</div></div>${nav()}`}
function filterList(){const q=document.getElementById('q').value.toLowerCase();let rs=currentCategory?recipes.filter(r=>r.category===currentCategory):recipes;rs=rs.filter(r=>r.name.toLowerCase().includes(q)||safeRecipeIngredients(r).join(' ').toLowerCase().includes(q));document.getElementById('cards').innerHTML=rs.map(card).join('')||'<div class="empty">Ingen resultater.</div>'}
function showRecipe(id){recipeViewContext=null;scale=1;renderRecipe(id)}
function recipeBackButton(r){if(recipeViewContext)return `<button class="btn secondary" onclick="returnFromRecipeContext()">← Madplan</button>`;return `<button class="btn secondary" onclick="library('${r.category}')">← ${r.category}</button>`}
function returnFromRecipeContext(){const source=recipeViewContext?.source||'current';recipeViewContext=null;scale=1;if(source==='draft')renderFlexibleDraft();else renderCurrentPlan()}
function combinePortionLines(groups){const map=new Map(),order=[];for(const lines of groups){for(const raw of (lines||[])){const q=parseIngredient(raw,1),key=q.name+'|'+q.unit;if(!map.has(key)){map.set(key,{...q,qty:q.qty||0,count:q.qty?0:1});order.push(key)}else{const v=map.get(key);if(q.qty)v.qty+=q.qty;else v.count+=1}}}return order.map(k=>{const v=map.get(k);if(v.qty)return fmtItem(v);if(v.count>1)return `${v.count} × ${v.raw}`;return v.raw})}
function recipeContextDetails(r){if(!recipeViewContext||recipeViewContext.recipeId!==r.id)return {label:r.portion,ingredients:r.ingredients,note:''};if(r.adultOnly){const ids=recipeViewContext.consumerIds||[];const allowed=ids.filter(id=>!r.audience||r.audience.includes(id));return {label:allowed.map(id=>flexPeople.find(p=>p.id===id)?.name).filter(Boolean).join(' + ')||r.portion,ingredients:r.ingredients,note:r.comfortOption?'Voksen-snack til den valgte profil.':'Personlig voksenopskrift til den valgte profil.'};}const ids=recipeViewContext.consumerIds||[],adults=ids.filter(x=>x==='alex'||x==='heidi').length,kids=ids.includes('kids');const data=portionData[r.id];const names=ids.map(id=>flexPeople.find(p=>p.id===id)?.name).filter(Boolean);let ingredients=r.ingredients,label=names.join(' + ')||r.portion,note='';if(data){if(adults===2&&kids)ingredients=data.family||r.ingredients;else if(adults===1&&!kids)ingredients=data.adult||r.ingredients;else if(adults===0&&kids)ingredients=data.kids2||r.ingredients;else if(adults===2&&!kids)ingredients=combinePortionLines([data.adult,data.adult]);else if(adults===1&&kids)ingredients=combinePortionLines([data.adult,data.kids2]);note='Mængderne er automatisk tilpasset de personer, der spiser denne ret i madplanen.'}else if(r.isCustom){note='Denne egen ret har ikke kuraterede portionsmængder endnu, så ingredienslisten vises som den er skrevet.'}return {label,ingredients,note}}
function showFlexRecipe(dayIndex,key,personId='shared',source='current'){const plan=flexPlanForSource(source);if(!plan)return;const st=flexMealState(plan.items[dayIndex],key);let recipeId='';if(st.mode==='shared')recipeId=st.sharedId;else recipeId=st.assignments[personId];if(!recipeId||recipeId===FLEX_OUT)return;let consumerIds=[];if(st.mode==='shared')consumerIds=flexPeople.map(p=>p.id);else consumerIds=flexPeople.filter(p=>st.assignments[p.id]===recipeId).map(p=>p.id);recipeViewContext={source,dayIndex,key,recipeId,consumerIds};scale=1;renderRecipe(recipeId)}
function renderRecipe(id){const r=recipes.find(x=>x.id===id);if(!r){library();return}const is=favs().includes(id),ctx=recipeContextDetails(r),ctxNote=ctx.note?`<div class="note portion-context-note"><b>Planlagt portion: ${escapeHtml(ctx.label)}</b><br>${ctx.note}</div>`:'';app.innerHTML=`<div class="shell">${siteHeader()}${recipeBackButton(r)}<article class="recipe"><button class="heart" onclick="toggleFav('${id}')">${is?'♥':'♡'}</button>${r.isCustom?`<div class="recipe-custom-actions"><button class="btn secondary small" onclick="customRecipeEditor('${id}')">Redigér egen ret</button><button class="text-link danger" onclick="deleteCustomRecipe('${id}')">Slet</button></div>`:''}${recipeMedia(r,'recipe-hero')}<span class="badge">${r.category}</span>${r.personalOwner?`<span class="custom-badge">${r.personalOwner==='alex'?'Alex-favorit':'Heidi-favorit'}</span>`:''}${r.comfortOption?'<span class="custom-badge">Kun Alex + Heidi</span>':''}${r.isCustom?'<span class="custom-badge">Familiens egen</span>':''}<h1>${escapeHtml(r.name)}</h1><div class="info"><div><b>Portion</b><br>${escapeHtml(ctx.label)}</div><div><b>Aktiv tid</b><br>${r.active}</div><div><b>Samlet tid</b><br>${r.total}</div></div>${ctxNote}<div class="portionbar"><button class="${scale===1?'active':''}" onclick="setScale('${id}',1)">${recipeViewContext?'Planlagt portion':'Kun i dag'}</button><button class="${scale===2?'active':''}" onclick="setScale('${id}',2)">Dobbelt portion</button></div><h3>Ingredienser ${scale===2?'· dobbelt portion':''}</h3><ul>${ctx.ingredients.map(x=>`<li>${scaleIngredient(x,scale)}</li>`).join('')}</ul><h3>Sådan gør du</h3><ol>${r.steps.map(x=>`<li>${x}</li>`).join('')}</ol>${r.taste?`<div class="note"><b>Sovs / dip / smag</b><br>${r.taste}</div>`:''}${r.child?`<div class="note pink" style="margin-top:10px"><b>Til børn på ca. 15 måneder</b><br>${r.child}</div>`:''}${r.tip?`<div class="note" style="margin-top:10px"><b>Praktisk tip</b><br>${r.tip}</div>`:''}<h3>${r.adultOnly?'Hvorfor passer den til denne voksen?':'Hvorfor er den god for børnene?'}</h3><ul>${(r.why||[]).map(x=>`<li>${x}</li>`).join('')}</ul>${feedbackHtml(id)}</article></div>${nav()}`;initFeedbackSelection(id)}
...
function favorites(){const rs=recipes.filter(r=>favs().includes(r.id));app.innerHTML=`<div class="shell">${siteHeader()}<div class="section-title"><h2>Favoritter</h2><button class="btn secondary" onclick="home()">← Tilbage</button></div><div class="cards">${rs.length?rs.map(card).join(''):'<div class="empty">Tryk ♡ på en opskrift for at gemme den her.</div>'}</div></div>${nav()}`}
function quickNow(){
  const cats=[['Morgenmad','☀️'],['Frokost','🥪'],['Aftensmad','🍲'],['Mellemmåltider','🍎']];
  app.innerHTML=`<div class="shell">${siteHeader()}<div class="section-title"><div><span class="eyebrow">Hurtig hjælp</span><h2>Jeg skal bruge noget hurtigt nu</h2></div><button class="btn secondary" onclick="home()">← Tilbage</button></div><p class="muted">Hvilket måltid står du og mangler? Kostkompasset finder en af de hurtigste retter i jeres eget opskriftsbibliotek.</p><div class="quick-category-grid">${cats.map(([c,e])=>`<button class="quick-category" onclick="quickMeal('${c}')"><span>${e}</span><strong>${c==='Mellemmåltider'?'Mellemmåltid':c}</strong><small>Find noget hurtigt</small></button>`).join('')}</div></div>${nav()}`;
}
function quickMeal(category){
  const limits={Morgenmad:10,Frokost:15,Aftensmad:20,Mellemmåltider:10};
  let list=recipes.filter(r=>r.category===category&&!r.adultOnly).filter(r=>minutes(r.active)<=limits[category]);
  if(!list.length)list=recipes.filter(r=>r.category===category&&!r.adultOnly).sort((a,b)=>minutes(a.active)-minutes(b.active)).slice(0,6);
...
function safeRecipeIngredients(r){return Array.isArray(r?.ingredients)?r.ingredients:[]}
function safeRecipeWhy(r){return Array.isArray(r?.why)?r.why:[]}
function hydrateRecipeRef(ref){
  if(!ref)return null;
  const id=typeof ref==='string'?ref:ref.id;
  if(id){const fresh=recipes.find(r=>r.id===id);if(fresh)return fresh}
  if(typeof ref==='object'&&ref.name){const fresh=recipes.find(r=>r.name===ref.name);if(fresh)return fresh;return {...ref,ingredients:safeRecipeIngredients(ref),why:safeRecipeWhy(ref)}}
  return null;
}
...
function normalizePlanStructure(plan){
  ...
  plan.days=Number(plan.days)||plan.items.length;
  if(!Array.isArray(plan.leftovers))plan.leftovers=[];
  plan.leftovers=plan.leftovers.filter(x=>x&&x.recipeId&&Number.isFinite(Number(x.sourceDayIndex))).map(x=>({...x,sourceDayIndex:Number(x.sourceDayIndex),used:!!x.used}));
  plan.items.forEach((d,i)=>{
    if(!d||typeof d!=='object'){plan.items[i]=d={day:i+1};}
    d.day=Number(d.day)||i+1;
    ['breakfast','lunch','dinner','snack','snackMorning','snackAfternoon'].forEach(k=>{if(d[k])d[k]=hydrateRecipeRef(d[k])});
    ...
    if(d.flex&&typeof d.flex==='object'){
      ...
      PLAN_MEAL_KEYS.forEach(k=>{
        const st=d.flex[k];if(!st||typeof st!=='object')return;
        if(!st.assignments||typeof st.assignments!=='object')st.assignments={alex:st.sharedId||d[k]?.id||'',heidi:st.sharedId||d[k]?.id||'',kids:st.sharedId||d[k]?.id||''};
      });
    }
    if(!d.outMeals||typeof d.outMeals!=='object')d.outMeals={};
    if(!d.manual||typeof d.manual!=='object')d.manual={};
  });
  return plan;
}
function planner(){
...
const flexPeople=[
  {id:'alex',name:'Alex',role:'1 voksen',initial:'A'},
  {id:'heidi',name:'Heidi',role:'1 voksen',initial:'H'},
  {id:'kids',name:'Sophia + Carlo',role:'2 små børn',initial:'S+C'}
];
const profileImagesKey='kostkompas-profile-images-v1';
const defaultProfileImages={alex:'profile-alex.png',heidi:'profile-heidi.png',kids:'profile-kids.png'};
function profileImages(){try{return JSON.parse(localStorage.getItem(profileImagesKey)||'{}')||{}}catch(e){return {}}}
function profileAvatar(person,editable=false){const src=profileImages()[person.id]||defaultProfileImages[person.id];return `<div class="profile-avatar-wrap">${editable?`<button class="profile-avatar ${src?'has-photo':''}" onclick="document.getElementById('profile-upload-${person.id}').click()" title="Skift profilbillede">${src?`<img src="${src}" alt="${person.name}">`:`<span>${person.initial}</span>`}</button><input id="profile-upload-${person.id}" class="profile-upload" type="file" accept="image/*" onchange="saveProfileImage('${person.id}',this)">`:`<div class="profile-avatar ${src?'has-photo':''}">${src?`<img src="${src}" alt="${person.name}">`:`<span>${person.initial}</span>`}</div>`}</div>`}
function saveProfileImage(personId,input){const file=input?.files?.[0];if(!file)return;if(file.size>1500000){alert('Vælg gerne et billede under 1,5 MB. Det lille profilfoto behøver ikke høj opløsning.');return}const reader=new FileReader();reader.onload=()=>{const all=profileImages();all[personId]=reader.result;localStorage.setItem(profileImagesKey,JSON.stringify(all));renderCurrentPlan()};reader.readAsDataURL(file)}
function flexProfileHeader(editable=true){return `<div class="flex-profile-grid"><div class="flex-profile-spacer"></div>${flexPeople.map(p=>`<div class="flex-profile-card">${profileAvatar(p,editable)}<strong>${p.name}</strong><small>${p.role}</small></div>`).join('')}</div>`}
function createBasePlanItems(days){
  const pattern=patterns[days],used=new Set(),out=[];
  for(let i=0;i<days;i++){
    const dinner=chooseDinner(pattern[i],used,'normal',i);used.add(dinner.id);
...
function generateFlexiblePlan(){
  try{
    const days=Number(document.getElementById('flex-days')?.value)||4;
  ...
  }catch(err){console.error('generateFlexiblePlan failed',err);alert('Madplansforslaget kunne ikke åbnes. Appen har nulstillet det midlertidige forslag – prøv knappen igen.');draftPlan=null;flexiblePlanner();}
}
const flexCollapsedDays={draft:new Set(),current:new Set()};
function flexCopyButton(dayIndex,key,source){return `<button class="mini-btn flex-copy-btn" onclick="openFlexCopyPicker(${dayIndex},'${key}','${source}')">⧉ Kopiér til andre dage</button>`}
function flexPlanBoard(plan,source,editableProfiles=false){normalizePlanStructure(plan);const collapsed=flexCollapsedDays[source]||new Set();return `<style>.flex-day-toggle{border:0;background:transparent;color:inherit;font:inherit;cursor:pointer;display:flex;align-items:center;gap:.5rem;padding:.2rem 0}.flex-day-toggle h3{margin:0}.flex-day-collapsed{padding-bottom:.65rem}.flex-day-body[hidden]{display:none!important}.flex-day-actions{display:flex;justify-content:flex-end;gap:.5rem;flex-wrap:wrap;margin-top:.8rem;padding-top:.7rem;border-top:1px solid rgba(24,70,57,.12)}.flex-copy-btn{margin-left:.45rem}.copy-day-list{display:grid;gap:.55rem;margin:1rem 0}.copy-day-option{display:flex;align-items:center;gap:.65rem;padding:.65rem .75rem;border:1px solid rgba(24,70,57,.15);border-radius:12px}.copy-day-option input{width:18px;height:18px}</style><div class="flex-plan-board">${flexProfileHeader(editableProfiles)}<div class="week flex-week">${plan.items.map((d,i)=>`<section class="day-card flex-day ${collapsed.has(i)?'flex-day-collapsed':''}"><div class="day-head"><button class="flex-day-toggle" onclick="toggleFlexDay(${i},'${source}')" aria-expanded="${collapsed.has(i)?'false':'true'}"><h3>${dayName(d.day,plan.startDate)}</h3><span>${collapsed.has(i)?'⌄':'⌃'}</span></button></div><div class="flex-day-body" ${collapsed.has(i)?'hidden':''}>${flexMealRow(d,i,'breakfast',source)}${flexMealRow(d,i,'snackMorning',source)}${flexMealRow(d,i,'lunch',source)}${flexMealRow(d,i,'snackAfternoon',source)}${flexMealRow(d,i,'dinner',source)}<div class="flex-day-actions"><button class="mini-btn choose-btn" onclick="openFlexCopyPicker(${i},'__day__','${source}')">⧉ Kopiér hele dagen</button><button class="mini-btn" onclick="toggleFlexDay(${i},'${source}')">Luk dagen</button></div></div></section>`).join('')}</div></div>`}
function toggleFlexDay(dayIndex,source='current'){const set=flexCollapsedDays[source]||(flexCollapsedDays[source]=new Set());if(set.has(dayIndex))set.delete(dayIndex);else set.add(dayIndex);rerenderFlexSource(source)}
function cloneFlexMealState(st){return JSON.parse(JSON.stringify(st||{}))}
function openFlexCopyPicker(dayIndex,key,source='current'){closeManualPicker();const plan=flexPlanForSource(source);if(!plan)return;const label=key==='__day__'?'hele dagen':keyLabel(key).toLowerCase();const overlay=document.createElement('div');overlay.id='manual-picker';overlay.className='picker-overlay';overlay.innerHTML=`<div class="picker-panel"><div class="picker-head"><div><div class="eyebrow">${dayName(plan.items[dayIndex].day,plan.startDate)}</div><h2>Kopiér ${label}</h2><p>Vælg de dage, der skal have de samme valg. De kan bagefter ændres helt frit.</p></div><button class="picker-close" onclick="closeManualPicker()">×</button></div><div class="copy-day-list">${plan.items.map((d,i)=>i===dayIndex?'':`<label class="copy-day-option"><input type="checkbox" class="flex-copy-target" value="${i}" checked><span>${dayName(d.day,plan.startDate)}</span></label>`).join('')}</div><button class="btn" onclick="applyFlexCopy(${dayIndex},'${key}','${source}')">Kopiér til valgte dage</button></div>`;overlay.addEventListener('click',e=>{if(e.target===overlay)closeManualPicker()});document.body.appendChild(overlay);document.body.style.overflow='hidden'}
function applyFlexCopy(dayIndex,key,source='current'){const plan=flexPlanForSource(source);if(!plan)return;const targets=[...document.querySelectorAll('.flex-copy-target:checked')].map(x=>Number(x.value));const src=plan.items[dayIndex];targets.forEach(i=>{const dst=plan.items[i];if(!dst)return;if(key==='__day__'){PLAN_MEAL_KEYS.forEach(k=>{dst[k]=src[k];dst.flex=dst.flex||{};dst.flex[k]=cloneFlexMealState(flexMealState(src,k));delete dst.flex[k].leftoverUseId;delete dst.flex[k].leftoverUseIds});dst.leftoverLunch=src.leftoverLunch;dst.makeDouble=src.makeDouble}else{dst[key]=src[key];dst.flex=dst.flex||{};dst.flex[key]=cloneFlexMealState(flexMealState(src,key));delete dst.flex[key].leftoverUseId;delete dst.flex[key].leftoverUseIds}});if(source!=='draft')persistPlan();closeManualPicker();rerenderFlexSource(source)}
...
function flexRecipe(id){if(!id)return null;return hydrateRecipeRef(typeof id==='string'?{id}:id)}
function flexPlanForSource(source){return source==='draft'?draftPlan:currentPlan}
function rerenderFlexSource(source){if(source==='draft')renderFlexibleDraft();else renderCurrentPlan()}
function leftoverTargetLabel(target){return target==='shared'?'hele familien':target==='kids'?'Sophia + Carlo':target==='alex'?'Alex':'Heidi'}
function leftoverId(dayIndex,key,target){return `lo-${dayIndex}-${key}-${target}`}
function sourceRecipeForFlex(plan,dayIndex,key,target){const d=plan?.items?.[dayIndex];if(!d)return null;const st=flexMealState(d,key);const id=target==='shared'?(st.mode==='shared'?st.sharedId:null):(st.mode==='individual'?st.assignments?.[target]:st.sharedId);return id&&id!==FLEX_OUT?flexRecipe(id):null}
function findSourceLeftover(plan,dayIndex,key,target){return (plan?.leftovers||[]).find(x=>x.sourceDayIndex===dayIndex&&x.sourceKey===key&&x.sourceTarget===target&&!x.removed)||null}
function leftoverUseId(st,target){if(!st)return '';if(target==='shared')return st.leftoverUseId||'';return st.leftoverUseIds?.[target]||''}
function refreshLeftoverUsage(plan,id){const lo=(plan?.leftovers||[]).find(x=>x.id===id);if(!lo)return;const refs=[];(plan.items||[]).forEach((d,dayIndex)=>PLAN_MEAL_KEYS.forEach(key=>{const st=flexMealState(d,key);if(st.leftoverUseId===id)refs.push({dayIndex,key,target:'shared'});Object.entries(st.leftoverUseIds||{}).forEach(([target,v])=>{if(v===id)refs.push({dayIndex,key,target})})}));lo.used=refs.length>0;if(refs.length)lo.usedBy=refs[0];else delete lo.usedBy}
function clearLeftoverUseForTarget(plan,d,key,target){const st=flexMealState(d,key);const id=leftoverUseId(st,target);if(target==='shared')delete st.leftoverUseId;else if(st.leftoverUseIds)delete st.leftoverUseIds[target];if(id)refreshLeftoverUsage(plan,id)}
function clearAllLeftoverUsesForMeal(plan,d,key){const st=flexMealState(d,key),ids=[st.leftoverUseId,...Object.values(st.leftoverUseIds||{})].filter(Boolean);delete st.leftoverUseId;delete st.leftoverUseIds;[...new Set(ids)].forEach(id=>refreshLeftoverUsage(plan,id))}
function availableLeftovers(plan,dayIndex,target){return (plan?.leftovers||[]).filter(lo=>!lo.removed&&!lo.used&&lo.sourceDayIndex<dayIndex&&(lo.sourceTarget==='shared'||lo.sourceTarget===target)).sort((a,b)=>b.sourceDayIndex-a.sourceDayIndex)}
function flexLeftoverSourceButton(dayIndex,key,target,source='current'){
  const plan=flexPlanForSource(source),lo=findSourceLeftover(plan,dayIndex,key,target);
  return `<button class="mini-btn ${lo?'choose-btn':''}" onclick="openSaveLeftover(${dayIndex},'${key}','${target}','${source}')">${lo?'✓ Rester gemt':'🥡 Gem som rester'}</button>`;
}
function flexLeftoverUseButton(dayIndex,key,target,source='current'){
  const plan=flexPlanForSource(source),d=plan?.items?.[dayIndex],st=d?flexMealState(d,key):null,id=leftoverUseId(st,target);
  return `<button class="mini-btn ${id?'choose-btn':''}" onclick="openLeftoverPicker(${dayIndex},'${key}','${target}','${source}')">${id?'🥡 Rester valgt':'🥡 Rester'}</button>`;
}
function openSaveLeftover(dayIndex,key,target,source='current'){
  closeManualPicker();const plan=flexPlanForSource(source);if(!plan)return;normalizePlanStructure(plan);const r=sourceRecipeForFlex(plan,dayIndex,key,target);if(!r){alert('Vælg først en ret til måltidet.');return}const existing=findSourceLeftover(plan,dayIndex,key,target);const overlay=document.createElement('div');overlay.id='manual-picker';overlay.className='picker-overlay';overlay.innerHTML=`<div class="picker-panel"><div class="picker-head"><div><div class="eyebrow">${dayName(plan.items[dayIndex].day,plan.startDate)} · ${keyLabel(key)}</div><h2>Gem som rester</h2><p>${escapeHtml(r.name)} · ${leftoverTargetLabel(target)}</p></div><button class="picker-close" onclick="closeManualPicker()">×</button></div><div class="copy-day-list"><button class="btn secondary" onclick="saveFlexLeftover(${dayIndex},'${key}','${target}','${source}',false)">Der er rester efter måltidet<small style="display:block;margin-top:.25rem">Ingen ekstra varer på indkøbslisten</small></button><button class="btn" onclick="saveFlexLeftover(${dayIndex},'${key}','${target}','${source}',true)">Lav dobbelt portion<small style="display:block;margin-top:.25rem">Den ekstra portion tælles med i indkøbslisten</small></button>${existing?`<button class="text-link danger" onclick="removeFlexLeftover('${existing.id}','${source}')">Fjern gemte rester</button>`:''}</div></div>`;overlay.addEventListener('click',e=>{if(e.target===overlay)closeManualPicker()});document.body.appendChild(overlay);document.body.style.overflow='hidden'
}
function saveFlexLeftover(dayIndex,key,target,source='current',extraPurchase=false){const plan=flexPlanForSource(source);if(!plan)return;normalizePlanStructure(plan);const r=sourceRecipeForFlex(plan,dayIndex,key,target);if(!r)return;let lo=findSourceLeftover(plan,dayIndex,key,target);if(!lo){lo={id:leftoverId(dayIndex,key,target),sourceDayIndex:dayIndex,sourceKey:key,sourceTarget:target,recipeId:r.id,used:false};plan.leftovers.push(lo)}lo.recipeId=r.id;lo.extraPurchase=!!extraPurchase;lo.removed=false;if(source!=='draft')persistPlan();closeManualPicker();rerenderFlexSource(source)}
function removeFlexLeftover(id,source='current'){const plan=flexPlanForSource(source);if(!plan)return;(plan.items||[]).forEach(d=>PLAN_MEAL_KEYS.forEach(key=>{const st=flexMealState(d,key);if(st.leftoverUseId===id)delete st.leftoverUseId;if(st.leftoverUseIds)Object.keys(st.leftoverUseIds).forEach(t=>{if(st.leftoverUseIds[t]===id)delete st.leftoverUseIds[t]})}));const lo=(plan.leftovers||[]).find(x=>x.id===id);if(lo){lo.removed=true;lo.used=false;delete lo.usedBy}if(source!=='draft')persistPlan();closeManualPicker();rerenderFlexSource(source)}
function openLeftoverPicker(dayIndex,key,target,source='current'){closeManualPicker();const plan=flexPlanForSource(source);if(!plan)return;normalizePlanStructure(plan);const d=plan.items[dayIndex],st=flexMealState(d,key),selected=leftoverUseId(st,target),list=availableLeftovers(plan,dayIndex,target);const overlay=document.createElement('div');overlay.id='manual-picker';overlay.className='picker-overlay';overlay.innerHTML=`<div class="picker-panel"><div class="picker-head"><div><div class="eyebrow">${dayName(d.day,plan.startDate)} · ${keyLabel(key)}</div><h2>Vælg rester</h2><p>Kun gemte rester fra tidligere dage vises her.</p></div><button class="picker-close" onclick="closeManualPicker()">×</button></div>${selected?`<button class="btn secondary" onclick="clearFlexLeftoverUse(${dayIndex},'${key}','${target}','${source}')">↩ Brug ikke rester her</button>`:''}<div class="picker-grid">${list.length?list.map(lo=>{const r=flexRecipe(lo.recipeId);return `<article class="picker-card">${r?recipeMedia(r,'picker-media'):''}<div class="picker-card-body"><div class="eyebrow">Fra ${dayName(plan.items[lo.sourceDayIndex].day,plan.startDate)} · ${keyLabel(lo.sourceKey)}</div><h3>${escapeHtml(r?.name||'Rester')}</h3><div class="meta">Oprindeligt til ${leftoverTargetLabel(lo.sourceTarget)}${lo.extraPurchase?' · planlagt dobbelt portion':''}</div><button class="btn" onclick="chooseFlexLeftover(${dayIndex},'${key}','${target}','${lo.id}','${source}')">Brug disse rester</button></div></article>`}).join(''):`<div class="empty"><b>Ingen gemte rester endnu.</b><br>Tryk “Gem som rester” på et tidligere måltid først.</div>`}</div></div>`;overlay.addEventListener('click',e=>{if(e.target===overlay)closeManualPicker()});document.body.appendChild(overlay);document.body.style.overflow='hidden'}
function chooseFlexLeftover(dayIndex,key,target,leftoverIdValue,source='current'){const plan=flexPlanForSource(source);if(!plan)return;const lo=(plan.leftovers||[]).find(x=>x.id===leftoverIdValue&&!x.removed&&!x.used);if(!lo)return;const r=flexRecipe(lo.recipeId);if(!r)return;const d=plan.items[dayIndex],st=flexMealState(d,key);if(target==='shared')clearAllLeftoverUsesForMeal(plan,d,key);else clearLeftoverUseForTarget(plan,d,key,target);if(target==='shared'){st.mode='shared';st.sharedId=r.id;st.assignments={alex:r.id,heidi:r.id,kids:r.id};st.leftoverUseId=lo.id;d[key]=r}else{if(st.mode==='shared'){const base=st.sharedId||d[key]?.id||'';st.mode='individual';st.assignments={alex:base,heidi:base,kids:base}}st.assignments[target]=r.id;if(!st.leftoverUseIds)st.leftoverUseIds={};st.leftoverUseIds[target]=lo.id}lo.used=true;lo.usedBy={dayIndex,key,target};if(source!=='draft')persistPlan();closeManualPicker();rerenderFlexSource(source)}
function clearFlexLeftoverUse(dayIndex,key,target,source='current'){const plan=flexPlanForSource(source);if(!plan)return;const d=plan.items[dayIndex];clearLeftoverUseForTarget(plan,d,key,target);if(source!=='draft')persistPlan();closeManualPicker();rerenderFlexSource(source)}
function flexPersonMealCell(st,p,dayIndex,key,source){const id=st.assignments[p.id];if(id===FLEX_OUT)return `<div class="flex-person-meal is-out"><div class="flex-out-name">🍽️ Spiser ude</div><button class="mini-btn meal-out-btn active" onclick="toggleFlexPersonOut(${dayIndex},'${key}','${p.id}','${source}')">↩ Spis hjemme</button></div>`;const r=flexRecipe(id),isLeft=!!leftoverUseId(st,p.id);return `<div class="flex-person-meal"><button class="flex-recipe-link" onclick="${r?`showFlexRecipe(${dayIndex},'${key}','${p.id}','${source}')`:`openFlexPicker(${dayIndex},'${key}','${p.id}','${source}')`}">${escapeHtml(r?.name||'Vælg ret')}</button>${isLeft?'<div class="meal-note">🥡 Rester · tælles ikke igen på indkøbslisten</div>':''}<div class="flex-person-tools"><button class="mini-btn" onclick="openFlexPicker(${dayIndex},'${key}','${p.id}','${source}')">Vælg</button>${flexLeftoverUseButton(dayIndex,key,p.id,source)}${r&&!isLeft?flexLeftoverSourceButton(dayIndex,key,p.id,source):''}<button class="mini-btn meal-out-btn" onclick="toggleFlexPersonOut(${dayIndex},'${key}','${p.id}','${source}')">🍽 Spiser ude</button></div></div>`}
function flexMealRow(d,dayIndex,key,source='current'){const st=flexMealState(d,key);if(st.mode==='shared'){const r=flexRecipe(st.sharedId)||d[key],isLeft=!!leftoverUseId(st,'shared');return `<div class="flex-meal-row shared"><div class="flex-meal-label">${keyLabel(key)}</div><div class="flex-shared-cell"><button class="flex-recipe-link" onclick="showFlexRecipe(${dayIndex},'${key}','shared','${source}')">${escapeHtml(r?.name||'Ikke planlagt')}</button>${isLeft?'<div class="meal-note">🥡 Rester · tælles ikke igen på indkøbslisten</div>':''}<div class="flex-row-tools"><button class="mini-btn" onclick="openFlexPicker(${dayIndex},'${key}','shared','${source}')">Vælg ret</button>${flexLeftoverUseButton(dayIndex,key,'shared',source)}${r&&!isLeft?flexLeftoverSourceButton(dayIndex,key,'shared',source):''}<button class="mini-btn choose-btn" onclick="splitFlexMeal(${dayIndex},'${key}','${source}')">Tilpas / spiser ude</button>${flexCopyButton(dayIndex,key,source)}</div></div></div>`}
  return `<div class="flex-meal-row individual"><div class="flex-meal-label">${keyLabel(key)}</div>${flexPeople.map(p=>flexPersonMealCell(st,p,dayIndex,key,source)).join('')}<div class="flex-collapse-tools"><button class="mini-btn choose-btn" onclick="collapseFlexMeal(${dayIndex},'${key}','${source}')">Alle får det samme</button>${flexCopyButton(dayIndex,key,source)}</div></div>`}
function toggleFlexPersonOut(dayIndex,key,personId,source='current'){const plan=flexPlanForSource(source);if(!plan)return;const d=plan.items[dayIndex],st=flexMealState(d,key);if(st.mode==='shared'){const id=st.sharedId||d[key]?.id||'',loId=st.leftoverUseId||'';st.mode='individual';st.assignments={alex:id,heidi:id,kids:id};if(loId){st.leftoverUseIds={alex:loId,heidi:loId,kids:loId};delete st.leftoverUseId;refreshLeftoverUsage(plan,loId)}}if(!st.lastHome)st.lastHome={};if(st.assignments[personId]===FLEX_OUT){st.assignments[personId]=st.lastHome[personId]||st.sharedId||d[key]?.id||''}else{clearLeftoverUseForTarget(plan,d,key,personId);st.lastHome[personId]=st.assignments[personId]||st.sharedId||d[key]?.id||'';st.assignments[personId]=FLEX_OUT}if(source!=='draft')persistPlan();rerenderFlexSource(source)}
function splitFlexMeal(dayIndex,key,source='current'){const plan=flexPlanForSource(source);if(!plan)return;const d=plan.items[dayIndex],st=flexMealState(d,key),id=st.sharedId||d[key]?.id||'',loId=st.leftoverUseId||'';st.mode='individual';st.assignments={alex:id,heidi:id,kids:id};if(loId){st.leftoverUseIds={alex:loId,heidi:loId,kids:loId};delete st.leftoverUseId;refreshLeftoverUsage(plan,loId)}if(source!=='draft')persistPlan();rerenderFlexSource(source)}
function collapseFlexMeal(dayIndex,key,source='current'){const plan=flexPlanForSource(source);if(!plan)return;const d=plan.items[dayIndex],st=flexMealState(d,key);const ids=Object.values(st.assignments).filter(id=>id&&id!==FLEX_OUT);const shared=ids[0]||st.sharedId||d[key]?.id||'',useVals=['alex','heidi','kids'].map(t=>st.leftoverUseIds?.[t]||'');const commonUse=useVals[0]&&useVals.every(x=>x===useVals[0])?useVals[0]:'';if(commonUse){delete st.leftoverUseIds;st.leftoverUseId=commonUse;refreshLeftoverUsage(plan,commonUse)}else clearAllLeftoverUsesForMeal(plan,d,key);st.mode='shared';st.sharedId=shared;st.assignments={alex:shared,heidi:shared,kids:shared};d[key]=flexRecipe(shared)||d[key];if(source!=='draft')persistPlan();rerenderFlexSource(source)}
function recipeAllowedForTarget(r,target){
  if(!r)return false;
  if(!Array.isArray(r.audience)||!r.audience.length)return true;
  if(target==='shared')return false;
  return r.audience.includes(target);
}
function recipesForFlexTarget(category,target){return recipes.filter(r=>r.category===category&&recipeAllowedForTarget(r,target))}
function personalFlexDefault(personId,key,dayIndex){
  const cat=categoryForPlanKey(key);
  const favorites=recipes.filter(r=>r.category===cat&&r.personalOwner===personId&&r.weekdayFavorite);
  if(!favorites.length)return null;
  if(personId==='alex'&&key==='breakfast')return favorites[dayIndex%favorites.length];
  if(personId==='heidi'&&key==='breakfast')return favorites.find(r=>r.id==='heidi-skyr-yoghurt-protein-chia')||favorites[0];
...
function activateDraftPlan(){
  if(!draftPlan)return;
  if(currentPlan?.items?.length)archiveCurrentPlan();
  currentPlan=draftPlan;draftPlan=null;shoppingChecks={};persistPlan();saveHouseholdStateLocal();renderCurrentPlan();
}
function quickShoppingHtml(recipe){
  loadHouseholdState();
  const groups={};(recipe.ingredients||[]).forEach(raw=>{const p=parseIngredient(raw,1);const c=categoryFor(p.name);(groups[c]??=[]).push(p)});
  const split=splitShoppingByPantry(groups),order=['Frugt & grønt','Fisk & kød','Mejeri & æg','Kolonial','Andet'];
  const need=order.flatMap(c=>(split.need[c]||[]).map(v=>({c,v}))),home=order.flatMap(c=>(split.home[c]||[]).map(v=>({c,v})));
...
function filterManualPicker(dayIndex,key){
  ...
  const rs=recipes.filter(r=>r.category===category&&recipeAllowedForTarget(r,'shared')&&(r.name.toLowerCase().includes(q)||safeRecipeIngredients(r).join(' ').toLowerCase().includes(q)));
...
function renderCurrentPlanCore(){
  closeManualPicker();
  ...
  app.innerHTML=`<div class="shell">${siteHeader()}
    <div class="section-title">
      <div><span class="eyebrow">Nuværende madplan</span><h2>${currentPlan.days}-dages madplan</h2></div>
    ...
    <div class="plan-actions">
      ...
      <button class="btn secondary" onclick="planner()">＋ Lav ny madplan</button>
    </div>
...
function renderCurrentPlan(){
  try{
    if(!currentPlan||!Array.isArray(currentPlan.items)||!currentPlan.items.length){planner();return}
    normalizePlanStructure(currentPlan);
    renderCurrentPlanCore();
  }catch(err){
    console.error('renderCurrentPlan failed',err);
    // Forsøg én sikker migrering fra den gemte plan og åbn igen.
    try{normalizePlanStructure(currentPlan);renderCurrentPlanCore();return}catch(e){console.error('renderCurrentPlan retry failed',e)}
    alert('Den gemte madplan er fra en ældre version og kunne ikke vises korrekt. Du kan lave en ny plan uden at miste opskrifter eller basislager.');
    planner();
  }
}
...
function nutritionCheck(plan){
  ...
  const text=all.map(r=>(r.name+' '+safeRecipeIngredients(r).join(' ')+' '+(r.why||[]).join(' ')).toLowerCase()).join(' ');
  ...
  const ironDays=plan.items.filter(d=>[['breakfast',d.breakfast],['snackMorning',d.snackMorning],['lunch',d.lunch],['snackAfternoon',d.snackAfternoon],['dinner',d.dinner]].filter(([k])=>!mealIsOut(d,k)).map(([,r])=>r).filter(Boolean).some(r=>/oksekød|kød|æg|laks|fisk|havre|linser|bønner|rugbrød/i.test((r.name+' '+safeRecipeIngredients(r).join(' '))))).length;
...
function normName(s){
  return s.toLowerCase()
    ...
    .replace(/\bcitronsaft\b|\bcitronjuice\b/gi,'citron')
    .replace(/\blimesaft\b|\blimejuice\b/gi,'lime')
    .replace(/\bmeget mørt grønt\b/gi,'broccoli')
    .replace(/\s+/g,' ')
    .trim();
}
...
function normalizeShoppingIngredient(p){
  const q={...p};
  // Saml citron/lime på tværs af stk, tsk, spsk og ml. Indkøbslisten viser hele frugter.
  if((q.name==='citron'||q.name==='lime')&&q.qty){
    const juiceMl=q.name==='citron'?45:30;
    if(q.unit==='tsk'){q.qty=(q.qty*5)/juiceMl;q.unit='stk'}
    else if(q.unit==='spsk'){q.qty=(q.qty*15)/juiceMl;q.unit='stk'}
    else if(q.unit==='ml'){q.qty=q.qty/juiceMl;q.unit='stk'}
  }
  return q;
}
function buildShopping(plan){
  ...
  if(plan.planType==='flex'){
    normalizePlanStructure(plan);plan.items.forEach((d,dayIndex)=>PLAN_MEAL_KEYS.forEach(key=>{
      const st=flexMealState(d,key);
      if(st.mode==='shared'){
        if(st.leftoverUseId)return;
        const extra=(plan.leftovers||[]).some(lo=>!lo.removed&&lo.sourceDayIndex===dayIndex&&lo.sourceKey===key&&lo.sourceTarget==='shared'&&lo.extraPurchase)?2:1;
        addStructured(flexRecipe(st.sharedId)||d[key],'family',extra);return
      }
      const ids=st.assignments||{},uses=st.leftoverUseIds||{};
      const hasIndividualExtra=(plan.leftovers||[]).some(lo=>!lo.removed&&lo.sourceDayIndex===dayIndex&&lo.sourceKey===key&&lo.extraPurchase&&['alex','heidi','kids'].includes(lo.sourceTarget));if(ids.alex&&ids.alex!==FLEX_OUT&&ids.alex===ids.heidi&&ids.alex===ids.kids&&!uses.alex&&!uses.heidi&&!uses.kids&&!hasIndividualExtra){addStructured(flexRecipe(ids.alex),'family');return}
      if(ids.alex&&ids.alex!==FLEX_OUT&&!uses.alex){const m=(plan.leftovers||[]).some(lo=>!lo.removed&&lo.sourceDayIndex===dayIndex&&lo.sourceKey===key&&lo.sourceTarget==='alex'&&lo.extraPurchase)?2:1;addStructured(flexRecipe(ids.alex),'adult',m)}
      if(ids.heidi&&ids.heidi!==FLEX_OUT&&!uses.heidi){const m=(plan.leftovers||[]).some(lo=>!lo.removed&&lo.sourceDayIndex===dayIndex&&lo.sourceKey===key&&lo.sourceTarget==='heidi'&&lo.extraPurchase)?2:1;addStructured(flexRecipe(ids.heidi),'adult',m)}
      if(ids.kids&&ids.kids!==FLEX_OUT&&!uses.kids){const m=(plan.leftovers||[]).some(lo=>!lo.removed&&lo.sourceDayIndex===dayIndex&&lo.sourceKey===key&&lo.sourceTarget==='kids'&&lo.extraPurchase)?2:1;addStructured(flexRecipe(ids.kids),'kids2',m)}
    }));
  }else{
    plan.items.forEach((d,i)=>{
...
function customRecipeEditor(id=''){
  ...
  app.innerHTML=`<div class="shell">${siteHeader()}<div class="section-title"><div><span class="eyebrow">Familiens egne retter</span><h2>${existing?'Redigér egen ret':'Tilføj jeres egen ret'}</h2></div><button class="btn secondary" onclick="library()">← Opskrifter</button></div>
