...
let recipes=[];let baseRecipes=[];let currentCategory=null;let scale=1;let currentPlan=null;let draftPlan=null;let recipeFeedback={};let pantryState={};let shoppingChecks={};let customRecipes=[];let planHistory=[];
...
function planner(){const saved=JSON.parse(localStorage.getItem('kostkompas-plan-settings')||'{"days":4}');app.innerHTML=`<div class="shell">${siteHeader()}<div class="section-title"><h2>Madplan</h2><button class="btn secondary" onclick="home()">← Tilbage</button></div><p class="muted">Motoren følger Kostsystemets rækkefølge og bruger nu også børnenes tidligere reaktioner og de seneste madplaner. Favoritter får et lille løft, mens nyligt brugte retter nedprioriteres – uden at afviste retter forsvinder permanent.</p><div class="planner-controls"><div class="control"><label>Antal dage</label><select id="days"><option ${saved.days==3?'selected':''}>3</option><option ${saved.days==4?'selected':''}>4</option><option ${saved.days==5?'selected':''}>5</option><option ${saved.days==7?'selected':''}>7</option></select></div><div class="control"><label>Familie</label><select disabled><option>2 voksne + 2 små børn</option></select></div></div><div class="plan-actions">${currentPlan?'<button class="btn" onclick="renderCurrentPlan()">📅 Nuværende madplan</button>':''}<button class="btn secondary" onclick="generatePlan()">✨ Lav forslag til ny madplan</button></div><div class="engine-note"><b>Personlig motor er aktiv.</b> Børnenes reaktioner påvirker forslagene nænsomt, og tidligere retter hjælper med variation.<br><br><b>Rester er aktive.</b> Når næste dags frokost bygges af aftensmaden, markeres aftensmaden som “lav dobbelt”, og frokosten tælles ikke dobbelt i indkøbslisten.</div></div>${nav()}`}
...
function generatePlan(){
  ...
  draftPlan={days,busy:'normal',items:out,created:Date.now()};
  renderDraftPlan()
}
