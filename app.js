...
let recipes=[];let baseRecipes=[];let portionData={};let currentCategory=null;let scale=1;let currentPlan=null;let draftPlan=null;let recipeFeedback={};let pantryState={};let shoppingChecks={};let customRecipes=[];let planHistory=[];
...
function renderFlexibleDraft(){if(!draftPlan||draftPlan.planType!=='flex'){flexiblePlanner();return}app.innerHTML=`<div class="shell">${siteHeader()}<div class="section-title"><div><span class="eyebrow">Forslag · fleksibel</span><h2>${draftPlan.days}-dages madplan</h2></div><button class="btn secondary" onclick="flexiblePlanner()">← Tilbage</button></div><div class="engine-note"><b>Din nuværende madplan er ikke ændret.</b> Startforslaget er roligt og fælles. Når du vælger “Brug denne madplan”, kan du åbne enkelte måltider og give Alex, Heidi eller Sophia + Carlo noget andet.</div><div class="plan-actions"><button class="btn" onclick="activateDraftPlan()">✓ Brug denne madplan</button><button class="btn secondary" onclick="generateFlexiblePlan()">↻ Lav et andet forslag</button>${currentPlan?'<button class="btn secondary" onclick="renderCurrentPlan()">📅 Behold nuværende</button>':''}</div>${flexProfileHeader(false)}<div class="week flex-week">${draftPlan.items.map(d=>`<section class="day-card flex-day"><div class="day-head"><h3>${dayName(d.day)}</h3></div>${['breakfast','lunch','dinner','snack'].map(k=>flexSharedPreview(k,d[k])).join('')}</section>`).join('')}</div></div>${nav()}`}
function flexSharedPreview(key,r){return `<div class="flex-meal-row shared"><div class="flex-meal-label">${keyLabel(key)}</div><div class="flex-shared-cell"><strong>${escapeHtml(r?.name||'Ikke planlagt')}</strong></div></div>`}
...
function flexMealRow(d,dayIndex,key){const st=flexMealState(d,key);if(st.mode==='shared'){const r=flexRecipe(st.sharedId)||d[key];return `<div class="flex-meal-row shared"><div class="flex-meal-label">${keyLabel(key)}</div><div class="flex-shared-cell"><button class="flex-recipe-link" onclick="showRecipe('${r?.id||''}')">${escapeHtml(r?.name||'Ikke planlagt')}</button><div class="flex-row-tools"><button class="mini-btn" onclick="openFlexPicker(${dayIndex},'${key}','shared')">Vælg ret</button><button class="mini-btn choose-btn" onclick="splitFlexMeal(${dayIndex},'${key}')">Del måltidet op</button></div></div></div>`}
  return `<div class="flex-meal-row individual"><div class="flex-meal-label">${keyLabel(key)}</div>${flexPeople.map(p=>{const r=flexRecipe(st.assignments[p.id]);return `<div class="flex-person-meal"><button class="flex-recipe-link" onclick="${r?`showRecipe('${r.id}')`:`openFlexPicker(${dayIndex},'${key}','${p.id}')`}">${escapeHtml(r?.name||'Vælg ret')}</button><button class="mini-btn" onclick="openFlexPicker(${dayIndex},'${key}','${p.id}')">Vælg</button></div>`}).join('')}<div class="flex-collapse-tools"><button class="mini-btn choose-btn" onclick="collapseFlexMeal(${dayIndex},'${key}')">Alle får det samme</button></div></div>`}
function splitFlexMeal(dayIndex,key){const d=currentPlan.items[dayIndex],st=flexMealState(d,key),id=st.sharedId||d[key]?.id||'';st.mode='individual';st.assignments={alex:id,heidi:id,kids:id};persistPlan();renderCurrentPlan()}
function collapseFlexMeal(dayIndex,key){const d=currentPlan.items[dayIndex],st=flexMealState(d,key);const ids=Object.values(st.assignments).filter(Boolean);const shared=ids[0]||d[key]?.id||'';st.mode='shared';st.sharedId=shared;st.assignments={alex:shared,heidi:shared,kids:shared};d[key]=flexRecipe(shared)||d[key];persistPlan();renderCurrentPlan()}
function openFlexPicker(dayIndex,key,target){closeManualPicker();const category=categoryForPlanKey(key),rs=recipes.filter(r=>r.category===category);const overlay=document.createElement('div');overlay.id='manual-picker';overlay.className='picker-overlay';overlay.innerHTML=`<div class="picker-panel"><div class="picker-head"><div><div class="eyebrow">${dayName(currentPlan.items[dayIndex].day)}</div><h2>Vælg ${keyLabel(key).toLowerCase()}</h2><p>${target==='shared'?'Til hele familien':target==='kids'?'Til Sophia + Carlo':target==='alex'?'Til Alex':'Til Heidi'}</p></div><button class="picker-close" onclick="closeManualPicker()">×</button></div><input class="search" id="flex-picker-q" placeholder="Søg…" oninput="filterFlexPicker(${dayIndex},'${key}','${target}')"><div class="picker-grid" id="flex-picker-grid">${rs.map(r=>flexPickCard(r,dayIndex,key,target)).join('')}</div></div>`;overlay.addEventListener('click',e=>{if(e.target===overlay)closeManualPicker()});document.body.appendChild(overlay);document.body.style.overflow='hidden'}
function flexPickCard(r,dayIndex,key,target){return `<article class="picker-card">${recipeMedia(r,'picker-media')}<div class="picker-card-body"><h3>${escapeHtml(r.name)}</h3><div class="meta">${r.active||''} aktiv · ${r.total||''}</div><button class="btn" onclick="chooseFlexMeal(${dayIndex},'${key}','${target}','${r.id}')">Vælg denne</button></div></article>`}
function filterFlexPicker(dayIndex,key,target){const q=(document.getElementById('flex-picker-q')?.value||'').toLowerCase(),category=categoryForPlanKey(key),rs=recipes.filter(r=>r.category===category&&(r.name.toLowerCase().includes(q)||r.ingredients.join(' ').toLowerCase().includes(q)));const grid=document.getElementById('flex-picker-grid');if(grid)grid.innerHTML=rs.map(r=>flexPickCard(r,dayIndex,key,target)).join('')||'<div class="empty">Ingen retter matcher søgningen.</div>'}
function chooseFlexMeal(dayIndex,key,target,recipeId){const d=currentPlan.items[dayIndex],st=flexMealState(d,key),r=flexRecipe(recipeId);if(!r)return;if(target==='shared'){st.mode='shared';st.sharedId=recipeId;st.assignments={alex:recipeId,heidi:recipeId,kids:recipeId};d[key]=r}else{st.mode='individual';st.assignments[target]=recipeId}persistPlan();closeManualPicker();renderCurrentPlan()}
function renderFlexibleCurrentPlan(){currentPlan.items.forEach(d=>{flexMealState(d,'breakfast');flexMealState(d,'lunch');flexMealState(d,'dinner');flexMealState(d,'snack')});const f=nutritionCheckFlexible(currentPlan),passed=f.filter(x=>x.ok).length;app.innerHTML=`<div class="shell">${siteHeader()}<div class="section-title"><div><span class="eyebrow">Nuværende madplan · fleksibel</span><h2>${currentPlan.days}-dages madplan</h2></div><button class="btn secondary" onclick="planner()">← Madplan</button></div><div class="plan-actions"><button class="btn" onclick="shoppingList()">🛒 Indkøbsliste</button><button class="btn secondary" onclick="flexiblePlanner()">＋ Lav ny fleksibel madplan</button></div>${flexProfileHeader(true)}<section class="nutrition-summary"><div><span class="eyebrow">Ernæringsfilter · Sophia + Carlo</span><h3>${passed} af ${f.length} pejlemærker ser gode ud</h3><p>Børnegruppen vurderes samlet, fordi Sophia + Carlo altid planlægges som 2 små børn.</p></div><div class="nutrition-chips">${f.slice(0,5).map(x=>`<span class="nutrition-chip ${x.ok?'ok':'attention'}">${x.ok?'✓':'•'} ${x.label}</span>`).join('')}</div></section><div class="engine-note"><b>Sådan læses planen:</b> Fælles ret står én gang på én linje hen over alle tre profiler. Kun når nogen får noget andet, åbner måltidet sig i tre kolonner.</div><div class="week flex-week">${currentPlan.items.map((d,i)=>`<section class="day-card flex-day"><div class="day-head"><h3>${dayName(d.day)}</h3></div>${flexMealRow(d,i,'breakfast')}${flexMealRow(d,i,'lunch')}${flexMealRow(d,i,'dinner')}${flexMealRow(d,i,'snack')}</section>`).join('')}</div><div class="section-title"><h2>Ernæringsfilter · Sophia + Carlo</h2></div><div class="filter-check">${f.map(x=>`<div class="check-card ${x.ok?'':'warn'}"><strong>${x.ok?'✓':'•'} ${x.label}</strong>${x.text}</div>`).join('')}</div></div>${nav()}`}
...
function buildShopping(plan){
  ...
  if(plan.planType==='flex'){
    plan.items.forEach(d=>['breakfast','lunch','dinner','snack'].forEach(key=>{
      ...
      if(ids.alex&&ids.alex===ids.heidi&&ids.alex===ids.kids){addPortion(flexRecipe(ids.alex),'family');return}
      if(ids.alex)addPortion(flexRecipe(ids.alex),'adult');
      if(ids.heidi)addPortion(flexRecipe(ids.heidi),'adult');
      if(ids.kids)addPortion(flexRecipe(ids.kids),'kids2');
    }));
  }else{
...
function shoppingList(){
  ...
  app.innerHTML=`<div class="shell">${siteHeader()}
    ...
    <p class="muted">${currentPlan.planType==='flex'?'Samme ingrediens samles på tværs af de tre profiler. De 80 Kostkompas-opskrifter skaleres med MASTER v3-portionsmængderne; egne retter beholder de mængder, I selv har skrevet.':'Samme ingrediens samles så vidt muligt på tværs af retterne. Planlagte restefrokoster tælles ikke dobbelt. Måltider markeret som “Spiser ude” fjernes automatisk.'} Flueben og basislager synkroniseres mellem jeres enheder.</p>
